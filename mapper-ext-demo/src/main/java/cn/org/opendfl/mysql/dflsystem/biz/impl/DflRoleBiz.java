package cn.org.opendfl.mysql.dflsystem.biz.impl;

import cn.org.opendfl.mysql.config.MybatisConfiguration;
...
import cn.org.opendfl.mysql.dflsystem.biz.IDflRoleBiz;
import cn.org.opendfl.mysql.dflsystem.mapper.DflRoleMapper;
...
public class DflRoleBiz extends BaseService<DflRolePo> implements IDflRoleBiz {
    ...
    public List<Long> selectIds(Integer status){
        System.out.println("----createTimeField="+mybatisConfiguration.getCreateTimeField());
        DflRolePo search=new DflRolePo();
        search.setIfDel(0);
        search.setStatus(status);
        return this.mapper.selectIds(search);
    }

    public Long selectMaxId(Integer status){
        DflRolePo search=new DflRolePo();
        search.setIfDel(0);
        search.setStatus(status);
        return this.mapper.selectMaxId(search);
    }
    ...
    public Integer selectCountExist(Integer status){
        DflRolePo search=new DflRolePo();
        search.setIfDel(0);
        search.setStatus(status);
        return mapper.selectCountExist(search);
    }

    public Integer selectCountByExample(Date endCreateTime) {
        Example example = new Example(DflRolePo.class);
        Example.Criteria criteria = example.createCriteria();
    ...
    public Integer selectCountExistByExample(Date endCreateTime) {
        Example example = new Example(DflRolePo.class);
        Example.Criteria criteria = example.createCriteria();
        criteria.andEqualTo("ifDel", 0);
        if(endCreateTime!=null){
        ...
        return mapper.selectCountExistByExample(example);
    }

    public DataCountVo selectCountAndMinMaxTimeByExample(Date endCreateTime) {
        Example example = new Example(DflRolePo.class);
        Example.Criteria criteria = example.createCriteria();
        criteria.andEqualTo("ifDel", 0);
        if(endCreateTime!=null){
        ...
        return mapper.selectCountAndMinMaxTimeByExample(example);
    }
}